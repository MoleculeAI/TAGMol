...
def compose_context_bond(h_protein, h_ligand_node, h_ligand_edge, pos_protein, pos_ligand, edge_index, batch_protein, batch_ligand_node, batch_ligand_edge):
    batch_node_ctx = torch.cat([batch_protein, batch_ligand_node], dim=0)
    sort_idx = torch.sort(batch_node_ctx, stable=True).indices

    # update the edge index after adding proteins
    # TODO possibility of a bug
    edge_index += h_protein.size(0)
    src, dst = edge_index
    src, dst = sort_idx[src], sort_idx[dst]
    edge_index = torch.tensor([src, dst])

    mask_ligand = torch.cat([
        torch.zeros([batch_protein.size(0)], device=batch_protein.device).bool(),
        torch.ones([batch_ligand_node.size(0)], device=batch_ligand_node.device).bool(),
    ], dim=0)[sort_idx]

    batch_node_ctx = batch_node_ctx[sort_idx]
    h_ctx = torch.cat([h_protein, h_ligand_node], dim=0)[sort_idx]  # (N_protein+N_ligand, H)
    pos_ctx = torch.cat([pos_protein, pos_ligand], dim=0)[sort_idx]  # (N_protein+N_ligand, 3)
    
    return h_ctx, pos_ctx, batch_node_ctx, mask_ligand, h_ligand_edge, edge_index, batch_ligand_edge
