...
def evaluate_for_pocket(sample_path, protein_path, docking_mode, docking_dir=None, exhaustiveness=16, verbose=False, eval_step=-1, eval_num_examples=None, save=True, atom_enc_mode='add_aromatic'):

    result_path = os.path.join(os.path.dirname(sample_path), 'eval_results')
    ...
    for example_idx, r_name in enumerate(tqdm(results_fn_list, desc='Eval')):
        ...
        for sample_idx, (pred_pos, pred_v) in enumerate(zip(all_pred_ligand_pos, all_pred_ligand_v)):
            ...
            try:
                ...
                if docking_mode == 'qvina':
                    with open(protein_path, 'r') as f:
                        pdb_block = f.read()
                    vina_task = QVinaDockingTask(pdb_block=pdb_block, ligand_rdmol=mol)
                    vina_results = vina_task.run_sync()
                elif docking_mode in ['vina_score', 'vina_dock']:
                    docking_dir = docking_dir if docking_dir else './tmp'
                    vina_task = VinaDockingTask(protein_path=protein_path, ligand_rdmol=mol, tmp_dir=docking_dir)
            ...
            results.append({
                'mol': mol,
                'smiles': smiles,
                'ligand_filename': "LIGAND_"+protein_path, #r['data'].ligand_filename,
                'pred_pos': pred_pos,
                'pred_v': pred_v,
...
if __name__ == '__main__':
    ...
    parser.add_argument('sample_path', type=str,help="path to *.pt file")
    ...
    parser.add_argument('--docking_dir',type=str,default=None)
    ...
    # result_path = os.path.join(os.path.dirname(args.sample_path), 'eval_results')
    # os.makedirs(result_path, exist_ok=True)
    # logger = misc.get_logger('evaluate', log_dir=result_path)
    # if not args.verbose:
    #     RDLogger.DisableLog('rdApp.*')
    ...
    # # Load generated data
    # results_fn_list = [args.sample_path]#glob(os.path.join(args.sample_path, '*.pt'))
    # # results_fn_list = sorted(results_fn_list, key=lambda x: int(os.path.basename(x)[:-3].split('_')[-1]))
    ...
    #                 with open(args.protein_path, 'r') as f:
    #                     pdb_block = f.read()
    #                 vina_task = QVinaDockingTask(pdb_block=pdb_block, ligand_rdmol=mol)
    #                 vina_results = vina_task.run_sync()
    ...
    #                 #     mol, r['data'].ligand_filename, protein_root=args.protein_root)
    #                 docking_dir = args.docking_dir if args.docking_dir else './tmp'
    #                 vina_task = VinaDockingTask(protein_path=args.protein_path, ligand_rdmol=mol, tmp_dir=docking_dir)
    ...
    #             'mol': mol,
    #             'smiles': smiles,
    #             'ligand_filename': "LIGAND_"+args.protein_path, #r['data'].ligand_filename,
    #             'pred_pos': pred_pos,
    #             'pred_v': pred_v,
    #             'chem_results': chem_results,
    #             'vina': vina_results
