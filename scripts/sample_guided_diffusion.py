...
def sample_guided_diffusion_ligand(model, guide_model, data, num_samples, kind=2, gradient_scale_cord=1.0, gradient_scale_categ=1e-10 , batch_size=16, device='cuda:0',
                            clamp_pred_min=None, clamp_pred_max=None,
    ...
    for i in tqdm(range(num_batch)):
        ...
        with torch.no_grad():
            ...
            r = model.sample_guided_diffusion(
                ...
                gradient_scale_categ=gradient_scale_categ,
...
if __name__ == '__main__':
    ...
    pred_pos, pred_v, pred_pos_traj, pred_v_traj, pred_v0_traj, pred_vt_traj, pred_pos0_traj, time_list  = sample_guided_diffusion_ligand(
        ...
        clamp_pred_min=config.sample.get("clamp_pred_min", None),
        clamp_pred_max=config.sample.get("clamp_pred_max", None),
